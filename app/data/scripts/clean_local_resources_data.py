...
def clean_local_resources_data():
  ...
  assert(set([math.isnan(value) for value in local_resources_df['Emergency Contact Phone Number'].values]) == {True})
  ...
  local_resources_df.rename(columns={'ID': 'id', 'Contact First Name': 'contact_first_name', 'Contact Last Name': 'contact_last_name', 'Company': 'company_name',                                    'Email': 'email_address', 'Preferred Contact Method': 'preferred_contact_method',                                    'Website': 'website', 'Address ID': 'address_id', 'Primary Phone Number': 'primary_phone_number',                                   'Secondary Phone Number': 'secondary_phone_number'}, inplace=True)


  # Add randomized availability IDs
  import random
  num_rows = local_resources_df['id'].count()
  local_resources_df['availability_id'] = [random.randint(0, 10) for i in range(num_rows)]
